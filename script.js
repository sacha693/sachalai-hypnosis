...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function showPricingTab(targetTab) {
        ...
        pricingButtons.forEach(button => {
            const isTarget = button.getAttribute('data-pricing') === targetTab;

            // 這裡使用 Tailwind CSS 的類名來設定活躍/非活躍狀態
            if (isTarget) {
                // 點擊的按鈕設為「活躍」樣式 (使用藍圖方案的顏色搭配)
                button.classList.add('active-pricing-tab', 'bg-amber-600', 'text-white', 'hover:bg-amber-700', 'shadow-xl');
                button.classList.remove('text-gray-700', 'bg-white', 'hover:bg-amber-50', 'shadow-md', 'border', 'border-gray-200');
            } else {
                // 其他按鈕設為「非活躍」樣式
                button.classList.remove('active-pricing-tab', 'bg-amber-600', 'text-white', 'hover:bg-amber-700', 'shadow-xl');
